...
data['Experience'] = data.groupby('Player')['Season'].transform('rank')
data['Experience_Squared'] = data['Experience'] ** 2

features.extend(['Age_Curve', 'Age_Squared', 'Experience', 'Experience_Squared'])
...
def create_improved_model(seq_length, n_features):
    ...
    x = Bidirectional(LSTM(256, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.003)))(inputs)
    x = LayerNormalization()(x)
    residual = x
    x = Bidirectional(LSTM(256, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.001)))(x)
    x = LayerNormalization()(x)
    x = Add()([x, residual])
    residual = x
    x = Bidirectional(LSTM(256, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.001)))(x)
...
improved_model.compile(optimizer=optimizer, loss='huber', metrics=['mae'])
...
improved_model.save('vorp_improved3.h5')
